...
Messages de conclusion de la journée ?

- data-driven versus process-based ?

- si on leur donne des jeux de données pour la journée 5: discuter avec eux des forces et faiblesses (biaias d'observations, que des donnes de présences). Comment faire avec des données imparfaites ?

- nombre de points de données versus dimensionalité du modèle ?

- notion de données effectives: e.g. distance suffisante entre les points d'échantillonages, sans autocorrélation.

Journée 5: trouver des jeux de données: 

Un graphe qui montre la quantité de données nécessaires versus complexité du modèle:
...
Faire des groupes dès le début la semaine (5 groupes)

On leur donne des papiers purement empiriques 

jeux de données (ou les leurs) ou des papiers empiriques à utiliser.

- 6 papiers avec process-based model et jeu de donnée déjà analysé ou papier purement empirique mais avec et questions de recherche et données dispo pour construire un modèle


Objectif de la journée 5: mise en pratique, avec emphase sur 

- répondre à une question commune en écologie à partir d'un jeu de données classique et d'un modèle

- donner des papiers purement empiriques, sans modèle. Pouvez-vous construire un modèle qui permet de conceptualiser les mécanismes qui sont discuter
    - Trouver des papiers (6) ou des jeux de données:résultats d'expériences par exemple.
